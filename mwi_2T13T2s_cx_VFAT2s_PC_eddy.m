...
%
%
function fitRes = mwi_2T13T2s_cx_VFAT2s_PC_eddy(algoPara,imgPara)
...
if numMagn==numel(te)   % magnitude fitting has 11 estimates
    ...
    estimates = zeros(ny,nx,nz,12+nfa*1); % others have 12 estimates
end
        ...
        for ky=1:ny
            parfor kx=1:nx
                if mask(ky,kx,kz)>0
                    ...
                    [estimates(ky,kx,kz,:),resnorm(ky,kx,kz)] = FitModel(s,fa,te,tr,b1,pini0,npulse,numMagn,isWeighted,weightMethod,isInvivo,userDefine,options,DEBUG);
                end
            end
...
function [x,res] = FitModel(s,fa,te,tr,b1,pini,npulse,numMagn,isWeighted,weightMethod,isInvivo,userDefine,options,DEBUG)
...
%     totalField0 = db0(:).';  totalFieldlb   = db0(:).'-100;                 totalFieldub    = db0(:).'+100;
    ...
    % set initial guess and fitting boundaries
    x0 = double([Amw0 ,Aiw0 ,Aew0 ,t2smw0 ,t2siw0 ,t2sew0 ,t1s0 ,t1l0 ,fmw0 ,fiw0 ,few0 ,kls0,pini0]);
    lb = double([Amwlb,Aiwlb,Aewlb,t2smwlb,t2siwlb,t2sewlb,t1slb,t1llb,fmwlb,fiwlb,fewlb,klslb,pinilb]);
    ub = double([Amwub,Aiwub,Aewub,t2smwub,t2siwub,t2sewub,t1sub,t1lub,fmwub,fiwub,fewub,klsub,piniub]);
end
