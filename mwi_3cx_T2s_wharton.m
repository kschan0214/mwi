...
if numMagn==numel(te)   % magnitude fitting has 8 estimates
    ...
    estimates = zeros(ny,nx,nz,11); % others have 10 estimates
end
        ...
        for ky=1:ny
            parfor kx=1:nx
                if mask(ky,kx,kz)>0
                    ...
                    db0 = fm(ky,kx,kz);
                    [estimates(ky,kx,kz,:),resnorm(ky,kx,kz)] = FitModel(s,te,b0,db0,numMagn,isWeighted,userDefine,options,DEBUG);
                end
            end
        end
    end
...
function [x,res] = FitModel(s,te,b0,db0,numMagn,isWeighted,userDefine,options,DEBUG)
if DEBUG
...
[x,res] = lsqnonlin(@(y)CostFunc(y,s,te,numMagn,isWeighted,b0,DEBUG),x0,lb,ub,options);

end

%% compute the cost function of the optimisation problem
function err = CostFunc(x,s,te,numMagn,isWeighted,b0,DEBUG)
...
if numMagn==numel(te) % magnitude fitting
    ...
    f_bkg=x(10);    pini=x(11);
end

% simulate signal based on parameter input
sHat = mwi_model_3cc_wharton2013(te, ...
                                 fvf, g, pd_m, e, ...
