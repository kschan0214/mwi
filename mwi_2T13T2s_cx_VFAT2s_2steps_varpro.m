...
%
%
function fitRes = mwi_2T13T2s_cx_VFAT2s_2steps(algoPara,imgPara)
...
if numMagn==numel(te)   % magnitude fitting has 10 estimates
    ...
    estimates_step1 = zeros(ny,nx,nz,5+nfa);
end
        ...
        for ky=1:ny
            for kx=1:nx
                if mask(ky,kx,kz)>0
                    ...
                    [estimates_step2(ky,kx,kz,:),resnorm_step2(ky,kx,kz)] = FitModel_vfa(s,fa,te,tr,b1,pini0,squeeze(estimates_step1(ky,kx,kz,:)),npulse,RFphi,numMagn,isWeighted,weightMethod,isInvivo,model,userDefine,options,DEBUG);
                end
            end
            if mod(ky,5) == 0;
                fprintf('%i ', ky);
...
if numMagn==numel(te) % magnitude fitting
    ...
    % set initial guess and fitting boundaries
    x0 = double([t2smw0 ,t2siw0 ,t2sew0 ,fmw0 ,fiw0 ,totalField0]);
    lb = double([t2smwlb,t2siwlb,t2sewlb,fmwlb,fiwlb,totalFieldlb]);
    ub = double([t2smwub,t2siwub,t2sewub,fmwub,fiwub,totalFieldub]);
...
if numMagn==numel(te) % magnitude fitting
    ...
    totalField0 = estimates(6:end).';	totalFieldlb   = estimates(6:end).'-100;	totalFieldub    = estimates(6:end).'+100;
