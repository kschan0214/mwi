...
fim_all2 = zeros(9,9,length(TR_r),nSpacing);
crlb_all2 = zeros(9,length(TR_r),nSpacing);
sigma = 0.5;
for kte = 1:length(TR_r)
    for kSpacing = 1:nSpacing
        t = te_r(kte,kSpacing).te;
        [fim, crlb] = mwi_crlb_3cc_mwf_Givenicvf(t,sigma,S0,mwf,icvf,r2s_mw,r2s_iw,r2s_ew,w_mb,w_ib,w_eb,phi);
        fim_all2(:,:,kte,kSpacing) = fim;
        crlb_all2(:,kte,kSpacing) = crlb;
    end
end
