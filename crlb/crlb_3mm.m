...
fim_all_3mm = zeros(6,6,length(TR_r),nSpacing);
crlb_all_3mm = zeros(6,length(TR_r),nSpacing);
sigma = 0.5;
for kte = 1:length(TR_r)
    for kSpacing = 1:nSpacing
        t = te_r(kte,kSpacing).te;
        [fim, crlb] = mwi_crlb_3mm_mwf(t,sigma,S0,mwf,icvf,r2s_mw,r2s_iw,r2s_ew);
        fim_all_3mm(:,:,kte,kSpacing) = fim;
        crlb_all_3mm(:,kte,kSpacing) = crlb;
    end
end
figure;imagesc(sqrt(squeeze(crlb_all_3mm(2,:,:))),[0 0.2]);