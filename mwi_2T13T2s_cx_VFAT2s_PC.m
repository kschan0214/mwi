...
%
%
function fitRes = mwi_2T13T2s_cx_VFAT2s_PC(algoPara,imgPara)
        ...
        for ky=1:ny
            parfor kx=1:nx
                if mask(ky,kx,kz)>0
                    ...
                    b1      = b1map(ky,kx,kz);
                    [estimates(ky,kx,kz,:),resnorm(ky,kx,kz)] = FitModel(s,fa,te,tr,b1,npulse,numMagn,isWeighted,weightMethod,isInvivo,userDefine,options,DEBUG);
                end
            end
...
function [x,res] = FitModel(s,fa,te,tr,b1,npulse,numMagn,isWeighted,weightMethod,isInvivo,userDefine,options,DEBUG)
...
if numMagn==numel(te) % magnitude fitting
    ...
    ub = double([Amwub,Aiwub,Aewub,t2smwub,t2siwub,t2sewub,t1sub,t1lub,fmwub,fiwub,fewub,klsub]);
end
