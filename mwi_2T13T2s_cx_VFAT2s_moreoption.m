...
isRFSpoiling = algoPara.fitting_option.rf_spoiling;
isExchange   = algoPara.fitting_option.exchange;
        ...
        for ky=1:ny
            parfor kx=1:nx
                if mask(ky,kx,kz)>0
                    ...
                    [estimates(ky,kx,kz,:),resnorm(ky,kx,kz)] = FitModel(s,fa,te,tr,b1,db0,pini0,npulse,numMagn,isWeighted,weightMethod,isInvivo,isRFSpoiling,isExchange,userDefine,options,DEBUG);
                end
            end
...
function [x,res] = FitModel(s,fa,te,tr,b1,db0,pini,npulse,numMagn,isWeighted,weightMethod,isInvivo,isRFSpoiling,isExchange,userDefine,options,DEBUG)
...
% run lsqnonlin!
[x,res] = lsqnonlin(@(y)CostFunc(y,s,fa,te,tr,b1,npulse,T3D_all,numMagn,isWeighted,weightMethod,isRFSpoiling,isExchange,DEBUG),x0,lb,ub,options);

end

%% compute the cost function of the optimisation problem
function err = CostFunc(x,s,fa,te,tr,b1,npulse,T3D_all,numMagn,isWeighted,weightMethod,isRFSpoiling,isExchange,DEBUG)
...
if numMagn==numel(te) % magnitude fitting
    ...
    if isExchange
        ...
        kls=0;
        totalfield = x(12:12+length(fa)-1);
        pini       = x(12+length(fa):end);
    end
end
...
sHat = mwi_model_2T13T2scc_epgx_moreoption(fa,te,tr,Amw,Aiw,Aew,t2smw,t2siw,t2sew,t1s,t1l,fmw,fiw,few,totalfield,pini,b1,kls,npulse,T3D_all,model);
...
try
    algoPara2.fitting_option.exchange = algoPara.fitting_option.exchange;
catch
    algoPara2.fitting_option.exchange = true;
end
try
    algoPara2.fitting_option.rf_spoiling = algoPara.fitting_option.rf_spoiling;
catch
    algoPara2.fitting_option.rf_spoiling = true;
end
