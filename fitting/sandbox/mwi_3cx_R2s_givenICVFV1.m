...
%
%
function fitRes = mwi_3cx_R2s_givenICVFV1(algoPara,imgPara)
...
if numMagn~=numel(te) % magnitude fitting
    ...
    E = 0.02;
    b0 = 3;
    freq_mw = zeros(1,length(theta0));
    freq_iw = zeros(1,length(theta0));
    for kfo = 1:length(theta0)
        [freq_mw(kfo),freq_iw(kfo)] = HCFM_FreqShift(theta0(kfo),x_i,x_a,g,E,b0);
    end
    freq_mw = sum(freq_mw(:).*ff0(:));
    freq_iw = sum(freq_iw(:).*ff0(:));
    
    fmwbg0   = (db0+freq_mw)*2*pi;         fmwbglb   = (db0-75)*2*pi; 	fmwbgub   = (db0+75)*2*pi;
    fiwbg0   = (db0+freq_iw)*2*pi;         fiwbglb   = (db0-25)*2*pi;  	fiwbgub   = (db0+25)*2*pi;
...
[x,res] = lsqnonlin(@(y)CostFunc(y,s,te,icvf0,numMagn,isWeighted,isNormCost,DEBUG),x0,lb,ub,options);

end

%% compute the cost function of the optimisation problem
function err = CostFunc(x,s,te,icvf,numMagn,isWeighted,isNormCost,DEBUG)
