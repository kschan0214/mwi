...
%
%
function fitRes = mwi_3cx_2T13T2s_givenICVF_HCFM(algoPara,imgPara)
...
freq_mw = zeros(size(theta));
freq_iw = zeros(size(theta));
for kfo = 1:size(theta,4)
[freq_mw(:,:,:,kfo),freq_iw(:,:,:,kfo)] = HCFM_FreqShift(theta(:,:,:,kfo),x_i,x_a,g,E,b0);
end
freq_mw = sum(freq_mw .* ff, 4);
freq_iw = sum(freq_iw .* ff, 4);
        ...
        for ky=1:ny
            parfor kx=1:nx
                if mask(ky,kx,kz)>0
                    ...
                    freq_mw0  = squeeze(freq_mw(ky,kx,kz));
                    freq_iw0     = squeeze(freq_iw(ky,kx,kz));
                    icvf0   = icvf(ky,kx,kz);
                    [estimates(ky,kx,kz,:),resnorm(ky,kx,kz)] = FitModel(s,fa,te,tr,b1,icvf0,freq_mw0,freq_iw0,db0,pini0,npulse,numMagn,isWeighted,weightMethod,isNormCost,isInvivo,userDefine,options,DEBUG);
                end
            end
            % display progress
...
function [x,res] = FitModel(s,fa,te,tr,b1,icvf,freq_mw0,freq_iw0,db0,pini,npulse,numMagn,isWeighted,weightMethod,isNormCost,isInvivo,userDefine,options,DEBUG)
...
fmw0 = freq_mw0;   fmwlb = freq_mw0-25; fmwub = freq_mw0+25;
fiw0 = freq_iw0;  fiwlb = freq_iw0-25; fiwub = freq_iw0+25;

% set initial guess and fitting boundaries
x0 = double([Amw0 ,Aiew0 ,t2smw0 ,t2siw0 ,t2sew0 ,t1s0 ,t1l0 ,kls0,fmw0,fiw0]);
lb = double([Amwlb,Aiewlb,t2smwlb,t2siwlb,t2sewlb,t1slb,t1llb,klslb,fmwlb,fiwlb]);
ub = double([Amwub,Aiewub,t2smwub,t2siwub,t2sewub,t1sub,t1lub,klsub,fmwub,fiwub]);
...
% run lsqnonlin!
[x,res] = lsqnonlin(@(y)CostFunc(y,s,fa,te,tr,b1,icvf,npulse,T3D_all,numMagn,isWeighted,weightMethod,isNormCost,DEBUG),x0,lb,ub,options);

end

%% compute the cost function of the optimisation problem
function err = CostFunc(x,s,fa,te,tr,b1,icvf,npulse,T3D_all,numMagn,isWeighted,weightMethod,isNormCost,DEBUG)
% capture all fitting parameters
Amw=x(1);   Aiew=x(2);  
t2smw=x(3)*1e-3; t2siw=x(4)*1e-3; t2sew=x(5)*1e-3;
t1s=x(6);   t1l=x(7);   kls=x(8);
freq_mw = x(9); freq_iw = x(10);
...
sHat = mwi_model_2T13T2scc_epgx(fa,te,tr,Amw,Aiw,Aew,t2smw,t2siw,t2sew,t1s,t1l,freq_mw,freq_iw,freq_ew,totalfield,pini,b1,kls,npulse,T3D_all);
...
numEst = 10; % basic setting has 8 estimates
