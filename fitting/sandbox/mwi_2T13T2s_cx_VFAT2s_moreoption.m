...
% Date last modified: 11 March 2019
%
%
function fitRes = mwi_2T13T2s_cx_VFAT2s_moreoption(algoPara,imgPara)
        ...
        for ky=1:ny
            parfor kx=1:nx
                if mask(ky,kx,kz)>0
                    ...
                    pini0   = pini(ky,kx,kz);
                    [estimates(ky,kx,kz,:),resnorm(ky,kx,kz)] = FitModel(s,fa,te,tr,b1,db0,pini0,RFphase,npulse,numMagn,isWeighted,weightMethod,isInvivo,model,userDefine,options,DEBUG);
                end
            end
            % display progress
...
function [x,res] = FitModel(s,fa,te,tr,b1,db0,pini,RFphase,npulse,numMagn,isWeighted,weightMethod,isInvivo,model,userDefine,options,DEBUG)
...
%     Aew0   = (3/9)*0.7*rho0; 	Aewlb   = 0;        Aewub   = 2*rho0;
    ...
    t2siw0 = 64e-3; 	t2siwlb = 25e-3;    t2siwub = 250e-3;
    t2sew0 = 48e-3; 	t2sewlb = 25e-3;    t2sewub = 250e-3;
...
% run lsqnonlin!
[x,res] = lsqnonlin(@(y)CostFunc(y,s,fa,te,tr,b1,npulse,RFphase,T3D_all,numMagn,isWeighted,weightMethod,model,DEBUG),x0,lb,ub,options);
