...
%
%
function fitRes = mwi_3cx_T2s_HCFM_MWFreq_givenDWI(algoPara,imgPara)
...
if numMagn==numel(te)   % magnitude fitting has 8 estimates
    ...
    estimates = zeros(ny,nx,nz,7); % others have 10 estimates
end
        ...
        for ky=1:ny
            parfor kx=1:nx
                if mask(ky,kx,kz)>0
                    ...
                    [estimates(ky,kx,kz,:),resnorm(ky,kx,kz)] = FitModel(s,te,db0,pini0,mcintra0,numMagn,isWeighted,userDefine,isInvivo,options,DEBUG);
                end
            end
        end
    end
...
function [x,res] = FitModel(s,te,db0,pini0,mcintra0,numMagn,isWeighted,userDefine,isInvivo,options,DEBUG)
if DEBUG
...
%     ub = double([Amwub,Aiwub,Aewub,t2smyub,t2saxub,theta0ub,fbkgub,piniub]);
    x0 = double([Amw0,Aiw0,t2smy0,t2sax0,theta0,fbkg0,pini0]);
    lb = double([Amwlb,Aiwlb,t2smylb,t2saxlb,thetalb,fbkglb,pinilb]);
    ub = double([Amwub,Aiwub,t2smyub,t2saxub,theta0ub,fbkgub,piniub]);
end
...
[x,res] = lsqnonlin(@(y)CostFunc(y,s,te,mcintra0,numMagn,isWeighted,DEBUG),x0,lb,ub,options);

end

%% compute the cost function of the optimisation problem
function err = CostFunc(x,s,te,mcintra,numMagn,isWeighted,DEBUG)
