...
%
%
function fitRes = mwi_2T13T2s_cx_VFAT2s_moreoption_givenDWI_gaussian(algoPara,imgPara)
        ...
        for ky=1:ny
            for kx=1:nx
                if mask(ky,kx,kz)>0
                    ...
                    db0     = squeeze(fm(ky,kx,kz,:));
                    pini0   = squeeze(pini(ky,kx,kz,:));
                    [estimates(ky,kx,kz,:),resnorm(ky,kx,kz)] = FitModel(s,fa,te,tr,b1,mcintra,db0,pini0,npulse,numMagn,isWeighted,weightMethod,isInvivo,model,userDefine,options,DEBUG);
                end
            end
            if mod(ky,5) == 0;
                fprintf('%i ', ky);
...
function [x,res] = FitModel(s,fa,te,tr,b1,mcintra,db0,pini,npulse,numMagn,isWeighted,weightMethod,isInvivo,model,userDefine,options,DEBUG)
...
if isInvivo
    ...
    t2prime0 = 200e-3; 	t2primelb = 0;    t2primeub = 2000e-3;
...
if numMagn==numel(te) % magnitude fitting
    ...
    fmw0   = 10;  	fmwlb   = -75;      fmwub   = 75;
    fiw0   = -2;  	fiwlb   = -10;  	fiwub   = 10;
    ...
    if strcmpi(model,'epgx')
        % set initial guess and fitting boundaries
%         x0 = double([Amw0 ,Aiw0 ,t2smw0 ,t2siw0 ,t2sew0 ,t1s0 ,t1l0 ,fmw0 ,fiw0  ,kls0,totalField0,pini0]);
%         lb = double([Amwlb,Aiwlb,t2smwlb,t2siwlb,t2sewlb,t1slb,t1llb,fmwlb,fiwlb,klslb,totalFieldlb,pinilb]);
%         ub = double([Amwub,Aiwub,t2smwub,t2siwub,t2sewub,t1sub,t1lub,fmwub,fiwub,klsub,totalFieldub,piniub]);
        x0 = double([Amw0 ,Aiw0 ,t2smw0 ,t2siw0 ,t2prime0 ,t1s0 ,t1l0 ,fmw0 ,fiw0  ,kls0,totalField0,pini0]);
        lb = double([Amwlb,Aiwlb,t2smwlb,t2siwlb,t2primelb,t1slb,t1llb,fmwlb,fiwlb,klslb,totalFieldlb,pinilb]);
        ub = double([Amwub,Aiwub,t2smwub,t2siwub,t2primeub,t1sub,t1lub,fmwub,fiwub,klsub,totalFieldub,piniub]);
    else
        % set initial guess and fitting boundaries
        x0 = double([Amw0 ,Aiw0 ,Aew0 ,t2smw0 ,t2siw0 ,t2sew0 ,t1s0 ,t1l0 ,fmw0 ,fiw0 ,few0 ,totalField0,pini0]);
        lb = double([Amwlb,Aiwlb,Aewlb,t2smwlb,t2siwlb,t2sewlb,t1slb,t1llb,fmwlb,fiwlb,fewlb,totalFieldlb,pinilb]);
        ub = double([Amwub,Aiwub,Aewub,t2smwub,t2siwub,t2sewub,t1sub,t1lub,fmwub,fiwub,fewub,totalFieldub,piniub]);
...
% run lsqnonlin!
[x,res] = lsqnonlin(@(y)CostFunc(y,s,fa,te,tr,b1,mcintra,npulse,RFphi0,T3D_all,numMagn,isWeighted,weightMethod,model,DEBUG),x0,lb,ub,options);
...
Amw=x(1);   Aiw=x(2);   Aew=Aiw*(1-mcintra)/mcintra;
t2smw=x(3); t2siw=x(4); t2sew=1./(1/t2siw + (1/x(5)));
t1s=x(6);   t1l=x(7);
fmw=x(8);   fiw=x(9);   
if numMagn==numel(te) % magnitude fitting
...
% simulate signal based on parameter input, 2-poolX
% [~,sHat_t1] = mwi_model_ssSPGR_2T1(fa*b1,tr,...
%                                       Amw,(Aiw+Aew),...
%                                       t1s,t1l,...
%                                       [],kls);
...
for ii=1:length(fa) 
    ...
    sHat_t1(1,ii) = (tmp{2}(end))*(Aiw+Aew+Amw);
    sHat_t1(2,ii) = (tmp{1}(end))*(Aiw+Aew+Amw);
...
sHat = zeros(size(s));
for kfa = 1:length(fa)
    Amw_curr = sHat_t1(1,kfa);          fmwbg = fmw + totalfield(kfa);
    Aiw_curr = sHat_t1(2,kfa)*mcintra;  fiwbg = fiw + totalfield(kfa);
    Aew_curr = sHat_t1(2,kfa)*(1-mcintra); 
   
    
    sHat(kfa,:) = mwi_model_3cc_guassian(te,Amw_curr,Aiw_curr,Aew_curr,t2smw,t2siw,t2sew,fmwbg,pd,sigma,fiwbg,totalfield(kfa),pini(kfa));
end
...
if DEBUG
    global DEBUG_resnormAll
    figure(99);
%     subplot(211);plot(te(:).',angle(permute(s,[2 1])),'^-');hold on;plot(te(:).',angle(permute(sHat,[2 1])),'x-.');hold off
    subplot(221);plot(te(:).',abs(permute(s,[2 1])),'^-');hold on;ylim([0-min(abs(s(:))),max(abs(s(:)))+10]);
    title('Magnitude');
    ...
    for kfa = 1:length(fa)
        text(te(1)/3,abs(s(kfa,1)),[num2str(fa(kfa))]);
    end
    plot(te(:).',abs(permute(sHat,[2 1])),'x-.');hold off
    subplot(222);plot(te(:).',angle(permute(s,[2 1])),'^-');hold on;
    plot(te(:).',angle(permute(sHat,[2 1])),'x-.');hold off
    title('phase');
    
    subplot(223);plot(te(:).',real(permute(sHat,[2 1])-permute(s,[2 1])),'o-.');hold on
    plot(te(:).',imag(permute(sHat,[2 1])-permute(s,[2 1])),'^--');
    hold off;title('Residual');
            
    
    DEBUG_resnormAll = [DEBUG_resnormAll;sum(err(:).^2)];
    subplot(2,2,4);plot(DEBUG_resnormAll);
