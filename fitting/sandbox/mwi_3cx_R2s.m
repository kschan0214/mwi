...
%
%
function fitRes = mwi_3cx_R2s(algoPara,imgPara)
...
if isInvivo
    ...
    Aax0   = 0.6*abs(s(1));  	Aaxlb   = 0;        Aaxub   = 2*abs(s(1));
    Aex0   = 0.3*abs(s(1));    	Aexlb   = 0;        Aexub   = 2*abs(s(1));
...
x0 = double([Amy0 ,Aax0 ,Aex0 ,r2smw0 ,r2siw0 ,r2sew0 ]);
lb = double([Amylb,Aaxlb,Aexlb,r2smwlb,r2siwlb,r2sewlb]);
ub = double([Amyub,Aaxub,Aexub,r2smwub,r2siwub,r2sewub]);
...
% t2smw=1/x(4);  t2siw=1/x(5); t2sew=1/x(6);
fmwbg=x(7)/(2*pi);  fiwbg=x(8)/(2*pi); 
if numMagn==numel(te) % magnitude fitting
    fbg=0;        pini=0;
else    % other fittings
    fbg=x(9)/(2*pi);     pini=x(10);
end
